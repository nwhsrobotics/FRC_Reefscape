...
public class AlternativePathfindAprilTag extends Command {
    ...
    @Override
    public boolean isFinished() { // right now the magic numbers are 0.158m and 0.168m (this is still big gap, try to get them as low as 0.04m and 0.08m or less)
        return Math.abs(VisionAprilTag.horizontalOffsetXAprilTag(vision.getLimelightName())) <= (Math.PI / 55) &&
